...
| `frontend/` | Next.js app (`frontend/src/`): landing page + login + gated `(app)` group (search / calendar / bookings / provider / account), real HTTP API seam | [frontend/CLAUDE.md](frontend/CLAUDE.md) |
| `plugin_sdk/` | The embeddable booking widget — `<script src=".../embed.js">` any third-party business can drop on their own site, backed by `frontend/`'s chromeless `/embed` route | [plugin_sdk/CLAUDE.md](plugin_sdk/CLAUDE.md) |
