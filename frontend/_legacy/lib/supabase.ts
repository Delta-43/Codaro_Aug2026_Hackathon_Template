 ...
 *  password flow -- see frontend/CLAUDE.md "Auth". The backend verifies the JWT
 *  this client issues; here we only need the anon key.
 *
 *  Env comes from NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY
 *  (see .env.local.example). If they're unset the client can't be built, so we
 *  return null and callers fall back gracefully (no session, no Bearer header)
