 ...
 * anchors; the Docs link is a real navigation, so it goes through next/link.
 * The primary button is auth-aware (Login → /login, or Open app → /search
 * when signed in); the logged-out button opens the plugin widget modal
 * (`window.Arbor.open()`) when it's loaded, falling back to a real
 * navigation to `/login` otherwise.
 *
  ...
  return (
    <nav className="fixed inset-x-0 top-4 z-50 flex justify-center px-4">
      <div className="flex w-full max-w-3xl items-center gap-2 rounded-3xl border border-border/60 bg-background/70 px-4 py-2 shadow-sm backdrop-blur-xl">
        ...
        {/* Far right — primary button (pinned), auth-aware */}
        <Link
